        ...
        if self.filename != None:
            ...
            if rdf != []:
                if self.Config.has_section(SEC_STRUCT) and self.Config.has_section(SEC_PARAMS):
                    ...
                    print "La structure du fichier n'est pas conforme à la version %s de l'application" % PASCAL_PARAM_VER
            else:
                self.fieldsCount = 1
...
#                        print "changes[i] = %s" % self.changes[fld]
                        ...
                        if self.changes[fld][1] != self.changes[fld][2]:
                            print "changement de format vers %s" % self.changes[fld][2]
                            if self.changes[fld][2] == "str":
                                ...
                                print "conversion vers combolist non développée"
                                # collecter les valeurs
                                # créer une nouvelle liste
                                # associer la liste au champ
                                pass
                            elif self.changes[fld][2] == "check":
                                if dnode0[self.changes[fld][0]+2].upper() == "NON":
                                    dnode[fld+2] = "False"
                                else :
                                    dnode[fld+2] = "True"
                    if  self.changes[fld][3] == "+" :
...
#                        print dnode
        ...
        if fname == None:
            return
        fp = open(fname,"w") # traiter le cas du nouveau fichier
    ...
    def remove_a_field(self, model, tree):
        ...
        if tvnode:
            ...
            print "suppression... changes = %s" % self.changes
    ...
    def remove_field(self, widget=None,model=None,tree=None):
        ...
        if self.fieldsCount > 1:    # we can't remove the last field
            if self.has_data :
                label = gtk.Label("Le fichier contient des données, confirmer vous la suppression de la colonne ?")
            ...
            print "Tentative de suppression d'un champ unique"
            pass
        
    def add_field(self, model):
        """Add a field with default format in the model"""
        model.append(['<champ>', 'str', 'str', False, True,'',''])
    
    def add_new_field(self, widget=None,model=None):
        """Adding field treatment"""
        self.add_field(model)
        if self.has_data == True:
            print "Ajout d'un champ sur une structure avec des données => ajouter la donnée par défaut"
    ...
    def add_list_choice(self, widget, fname):
        print "Ajouter une liste de choix"
...
#        ListTab(fname, "list"+str(lst), self.choicelist)
    ...
    def drag_data_received_data(self, treeview, context, x, y, selection, info, etime):
        ...
        if drop_info:
            ...
            print "drag %s" % self.dragpos
            print "position %s" % position
            if position != gtk.TREE_VIEW_DROP_AFTER:
#            if (position == gtk.TREE_VIEW_DROP_BEFORE or position == gtk.TREE_VIEW_DROP_INTO_OR_BEFORE):
                model.insert_before(iterc, datadrop)
                print "drop avant %s" % path
            else:
                model.insert_after(iterc, datadrop)
                print "drop après %s" % path
                self.droppos +=1
            print "déplacer %s" % self.dragpos
            print "avant %s" % self.droppos
    ...
    def __init__(self,fname = None, extcall = False):
        ...
        if extcall == False:
            self.filename = PaChooser() #open à file or not
...
#                                  buttons=(gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL,gtk.STOCK_SAVE, gtk.RESPONSE_OK))
        self.window = gtk.Dialog(title=u"Paramétrage des données", parent=None, flags=gtk.DIALOG_MODAL, buttons=None)
...
#            button.connect("clicked", self.def_choice,nblst+1, self.filename)
        ...
        self.column.append(gtk.TreeViewColumn('champ', self.renderer[0], text=0))
        self.column.append(gtk.TreeViewColumn('nature', self.renderer[1], text=1))
        self.column.append(gtk.TreeViewColumn('type', self.renderer[2], text=2))
        self.column.append(gtk.TreeViewColumn('modifiable', self.renderer[3]))
        self.column[3].add_attribute( self.renderer[3], "active", 3)
        self.column.append(gtk.TreeViewColumn('visible', self.renderer[4]))
        self.column[4].add_attribute( self.renderer[4], "active", 4)
        self.column.append(gtk.TreeViewColumn("fonction d'édition", self.renderer[5], text=5))
        self.column.append(gtk.TreeViewColumn('liste de choix', self.renderer[6], text=6))
...
class ListTab:
    def __init__(self, fname, listname, choicelist):
        ...
        self.wndList = gtk.Dialog(title=u"Liste : "+listname, parent=None, flags=gtk.DIALOG_MODAL, buttons=None)
...
#        return
    ...
    def save_list(self, widget, model, tree, fname, listname):
        ...
        dnode=[]
        nbitem = 0
        print "sauvegarde de la liste %s" % listname
        print "...dans le fichier %s " % fname
        if fname != None :
            Config = ConfigParser.ConfigParser()
            Config.read(fname)
...
class ParamsGlob:
    def __init__(self, extcall = False):
        ...
        self.wndPGlob = gtk.Dialog(title=u"Paramétrage de l'application", parent=None, flags=gtk.DIALOG_MODAL, buttons=None)
        ...
        # title edition
        label = gtk.Label("Nom de l'application : ")
        label.set_alignment(1,0)
        entrytitle = gtk.Entry()
        entrytitle.set_max_length(50)
        ...
        label = gtk.Label("Version de l'application : ")
        label.set_alignment(1,0)
    ...
    def saveglob(self, widget, extcall=False):
        ...
        if rdf != [] and Config.has_section(SEC_INI_OTHER) == True:
            print "...de %s" % self.application_name
            Config.set(SEC_INI_OTHER,ITM_APPNAME,self.application_name)
            print Config.get(SEC_INI_OTHER,ITM_APPNAME)
            cfgfile = open(INIFILE,'w')
