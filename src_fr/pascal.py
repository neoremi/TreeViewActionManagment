...
def menu_toolbar_create():
        ...
        menu_file = gtk.Menu()    
#        menu_item_file = gtk.MenuItem(_("_Fichier"))    #RHE20160926 "_" linked to duplicity : what is the interest ?
        ...
        actions.opn.connect ("activate", fct_open, "Ouverture")
...
#        menu_item_saveAs.add_accelerator("activate", app_win.accel_group, ord("S"), gtk.gdk.CONTROL_MASK, gtk.ACCEL_VISIBLE)
        ...
        actions.imp = gtk.Action("Import", "Import", "Convert CSV to PA file", None)
        ...
        menu_item_struct = gtk.MenuItem("Structure")
...
def fct_renew(widget, event=None):
    ...
    print "Création d'un nouveau fichier avec la même structure"
    if app_win.curPA > -1:
        saveas(widget, event, False)
    else:
        buf = "La réinitialisation des données n'est possible que s'il y a un plan actif."
        app_win.Status.push_item(app_win.Status.status_bar,app_win.Status.context_id,buf)

def fct_new(widget, event=None):
    print "Création d'une nouvelle structure de plan"
...
def fct_open(widget, string, fPA=None):
    app_win.Status.pop_item(app_win.Status.status_bar,app_win.Status.context_id)    #empty status stack
    if fPA == None:
        file_dialog = gtk.FileChooserDialog(_("Ouvrir..."), app_win, gtk.FILE_CHOOSER_ACTION_OPEN, (gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL, gtk.STOCK_OPEN, gtk.RESPONSE_OK))
        ...
        if response == gtk.RESPONSE_OK:
            ...
            if OpenDataFile(fPA,app_win.maxPA):  #data loading
                DataToNotebook(app_win.maxPA-1, app_win.maxPA-1)
                buf = "Ouverture du fichier '%s'" % file_dialog.get_filename()
                app_win.Status.push_item(app_win.Status.status_bar,app_win.Status.context_id,buf)
        file_dialog.destroy()
    else:
        if OpenDataFile(fPA,app_win.maxPA):  #data loading
...
def fct_import(widget, string, fPA=None):
    ...
    if paName != None:
        if OpenDataFile(paName,app_win.maxPA):  #data loading
            DataToNotebook(app_win.maxPA-1, app_win.maxPA-1)
            buf = "Import et ouverture du fichier '%s'" % paName
            app_win.Status.push_item(app_win.Status.status_bar,app_win.Status.context_id,buf)
...
class BarreOutil:
    ...
    def add_action(self,widget=None,event=None,code=None,data=None, curpa=None):
        ...
        if self.inc_auto_id(curpa) == False:
            buf = "One more node can't be added because of id limitation"
            app_win.Status.push_item(app_win.Status.status_bar,app_win.Status.context_id,buf)
            print buf
            return
        ...
        if code != None:
            ...
            if match_iter:
                buf = "WARNING : The node %s already exists! It can't be added." % data[fmodel.idpos]
                app_win.Status.push_item(app_win.Status.status_bar,app_win.Status.context_id,buf)
                print buf
                return    
...
#            newnode.append(data[fmodel.idpos])
    ...
    def add_next(self,widget,event=None,curpa=None):
        #print "Ajout d'une action de même niveau"
        if curpa == None:   # if addition is request by button (not by file loading)
            app_win.curPA = app_win.notebook.get_current_page()
            curpa=app_win.curPA
    ...
    def add_detail(self,widget=None,event=None,code=None,data=None,curpa=None):
        "Add an action to detail a 'mother' action"
        if curpa == None:   # if addition is request by button (not by file loading)
            app_win.curPA = app_win.notebook.get_current_page()
            curpa=app_win.curPA
        app_win.Status.pop_item(app_win.Status.status_bar,app_win.Status.context_id)
        ...
        if code == None:
            pass
            #print "Ajout d'une nouvelle action de détail"
        else:
            pass
            #print "Ajout de l'action de détail %s" % code
        if code != None:
            #print "recherche de %s" % data[fmodel.idpos+INTDATACOUNT]
            match_iter = self.search(model, model.iter_children(None),self.match_func, (fmodel.idpos, data[fmodel.idpos+INTDATACOUNT]))
            if match_iter:
                buf = "WARNING : The node %s already exists! It can't be added." % data[fmodel.idpos]
                app_win.Status.push_item(app_win.Status.status_bar,app_win.Status.context_id,buf)
...
#                print "newnode %s" % newnode
                ...
                buf = "WARNING : Mother node %s hasn't be found!" % data[MOTHERPOS]
                app_win.Status.push_item(app_win.Status.status_bar,app_win.Status.context_id,buf)
                print buf
        else:
            TreeSelection = fmodel.Display.view.get_selection()
    ...
    def inc_auto_id(self,curpa=0):
        ...
        if fmodel.IDmax < fmodel.IDroof:
            #Tester availability of the default next code
...
#        print "recherche dans %s de %s" % (model,data)
        while tvnode :
    ...
    def select_ID(self,widget,saisie,curpa=0):
        ...
        #print "Selection de l'item %s" % selID
    ...
    def radio_event(self, widget, toolbar):
        ...
        if self.button1.get_active():
            pass
            #print "Bouton radio 1 activé"
        elif self.button2.get_active():
            pass
            #print "Bouton radio 2 activé"

    def toggle_event(self, widget, toolbar):
        """traitement de la bascule de position de bouton"""
        pass
        #print "bascule effectuée"
    ...
    def __init__(self):
        ...
        # Création du bouton d'ajout principal
        iconw = gtk.Image() # icon widget
        iconw.set_from_file("../img/add1.xpm")
        add1_button = self.toolbar.append_item(
            "Nouveau",           # label du bouton
            "Ajouter un élément", # this button's tooltip
            "Private",         # tooltip private info
            iconw,             # icon widget
            self.add_action,    # signal
            app_win.curPA)   # rank of open object

        # Création du bouton d'ajout après l'action courante
        iconw = gtk.Image() # icon widget
        iconw.set_from_file("../img/add.xpm")
        addPlus_button = self.toolbar.append_item(
            "Ajouter",           # label du bouton
            "Ajouter un élément suivant", # this button's tooltip
            "Private",         # tooltip private info
            iconw,             # icon widget
            self.add_next,0) # signal

        # Création du bouton d'ajout d'une action de détail
        iconw = gtk.Image() # icon widget
        iconw.set_from_file("../img/add2.xpm")
        addDetail_button = self.toolbar.append_item(
            "Détailler",           # label du bouton
            "Ajouter un élément de détail", # this button's tooltip
            "Private",         # tooltip private info
            iconw,             # icon widget
            self.add_detail,0) # signal

        # Création du bouton de suppression d'une action
        iconw = gtk.Image() # icon widget
        iconw.set_from_file("../img/suppr.xpm")
        suppr_button = self.toolbar.append_item(
            "Supprimer",           # label du bouton
            "Supprimer un élément sélectionné", # this button's tooltip
            "Private",         # tooltip private info
            iconw,             # icon widget
...
class InfoModel:
    ...
    def __init__(self,fmodel):
        ...
        return
    def get_model(self):
        """ Retourner le modèle pour attachement à la vue"""
        if self.tree_store:
...
class DisplayModel:
    ...
    def make_view( self, curpa=0 ):
        """ Construction d'une vue pour l'arborescence """
        model = app_win.FileOpen[curpa].Store.tree_store
        fmodel = app_win.FileOpen[curpa]
...
#                    loadPA(fmodel, 3, chlist)
    ...
    def col_edited_cb( self, cell, path, new_text, data ):
        """
        Traitement de modification (affectation de la nouvelle donnée dans l'arborescence).
        """
    ...
    def col_toggled_cb( self, cell, path, data ):
        """Enregistrement de la bascule du bouton/case à cocher..."""
        model,col = data
        model[path][col] = not model[path][col]
        #print "Changement de '%s' en %s" % (model[path][0], model[path][col])
        return
    def col_edited_lb( self, cell, path, choix, data ):
        """Enregistrement du choix dans la combo."""
        model,col = data
        model[path][col] = choix
        #print "Bascule de '%s' en %s" % (model[path][col], choix)
        return
...
class FileModel:
    ...
    def destroy_cb(self, *kw):
        """ Destroy callback pour fermer l'application """
        writeIni(app_win.FileOpen[app_win.curPA],app_win.initFile,app_win.dico)
        #print "sauvegarde des paramètres dans %s" % fichIni
        gtk.main_quit()
        return

    def run(self):
        """ run est appelé pour démarrer le gestionnaire d'événement GTK """
        gtk.main()
        return    
...
def apply_ini():
    ...
    if structOK:
        ...
        buf = "Fichier 'pascal.ini' : " + ErrMess
        app_win.Status.push_item(app_win.Status.status_bar,app_win.Status.context_id,buf)
...
def loadPA(filepos, loadstep, section=None):
    ...
    if ctrlOK == False:
        buf = "Contrôle de format : message d'erreur : %s" % ErrMess
        print buf
        app_win.Status.push_item(app_win.Status.status_bar,app_win.Status.context_id,buf)
...
#                        print "ajout détail %s" % opt
            ...
            for s in [SEC_STRUCT,SEC_DATA,SEC_PARAMS]:
                if Config.has_section(s) == False:
                    print "pas les sections %s" % s

def saveBranch(model, fmodel, tvnode, Config):
    """Fonction de sauvegarde d'une branche de l'arbre"""
    while tvnode :
        #save current node
        dnode=[]
        for col in range(fmodel.fieldsCount):
...
def savePA(model, fmodel, fPA, fPADest=None, saveData=True):
        ...
        #RHE traiter n liste (dans le paramétrage...............................
        #=======================================================================
        ...
        Config.write(fp)
        fp.close()
        buf = "Sauvegarde du fichier sous '%s'" % fPADest
        app_win.Status.push_item(app_win.Status.status_bar,app_win.Status.context_id,buf)
        ...
        if fPA != fPADest:
            ...
            simplyclose(None, confirm=False)
            # reload saved plan
            print "Chargement de %s" % fPADest
            fct_open(None, "Import", fPADest)
            pass

def save(widget, event=None):
    app_win.Status.pop_item(app_win.Status.status_bar,app_win.Status.context_id)    #empty status stack
    buf = "Enregistrement de %s" % app_win.FileOpen[app_win.curPA].PAFile
    print buf
...
def saveas(widget, event=None, saveData=True):
    """Save plan as .pa"""
    app_win.Status.pop_item(app_win.Status.status_bar,app_win.Status.context_id)
    file_dialog = gtk.FileChooserDialog(_("Enregistrer sous..."), app_win, gtk.FILE_CHOOSER_ACTION_SAVE, (gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL, gtk.STOCK_SAVE, gtk.RESPONSE_OK))
    file_dialog.set_default_response(gtk.RESPONSE_OK)
    filter = gtk.FileFilter()
    ...
    if response == gtk.RESPONSE_OK:
        ...
        for i in range(app_win.maxPA):
            if app_win.FileOpen[i].PAFile == new_name:
                print "Sauvegarde refusée : le fichier %s est déjà ouvert" % new_name
                file_dialog.destroy()
                return False
        print "Sauvegarde de %s" % new_name
...
def simplyclose(widget, event=None, confirm=True):
    ...
    if confirm == True:
        label = gtk.Label("Voulez-vous vraiment fermer ce plan sans le sauvegarder ?")
        confirm_dialog = gtk.Dialog(title="Confirmation", flags=gtk.DIALOG_MODAL, buttons=(gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL, gtk.STOCK_OK, gtk.RESPONSE_OK))
...
def change_pa(self, page, page_num):
    """Store current PA """
    #print "page initiale : %s" % app_win.notebook.get_current_page()
    #print "nouvelle page : %s" % page_num
...
#    print "OpenDataFile %s" % datafile+" en position "+str(filepos)
    ...
    for i in range(app_win.maxPA):
        if datafile == app_win.FileOpen[i].PAFile:
            buf = "Le fichier %s est déjà ouvert" % datafile
            print buf
            app_win.Status.push_item(app_win.Status.status_bar,app_win.Status.context_id,buf)
            return False
    FModel = FileModel()
#    print "avant ajout : %s" % app_win.FileOpen
    app_win.FileOpen.append(FModel)             #add a data model to the application
#    print "après ajout : %s" % app_win.FileOpen
    app_win.maxPA += 1
...
#    app_win.set_default_size(800, 600) 
    app_win.set_size_request(1000, 600) # taille de la fenêtre à l'ouverture
    try:
    ...
    if app_win.dico["lastFile"] != "":
        if OpenDataFile(app_win.dico["lastFile"],0):  #data loading
            DataToNotebook(0, 0)
            buf = "Chargement du fichier : '%s'" % app_win.FileOpen[0].PAFile
            app_win.Status.push_item(app_win.Status.status_bar,app_win.Status.context_id,buf)
